...
"""install_seedgen.py — sets up a fresh Raspberry Pi (OS bookworm, Pi 5)
completely for the QUANTUM SEED GENERATOR.

Performs:
  1. System packages:  python3-lgpio, python3-smbus2, i2c-tools, rfkill,
     alsa-utils, qrencode
  2. Enable the I2C interface (for the MLX90640)
  3. Configure GPIO17 permanently bias-free (gpio=17=ip,pn in config.txt)
     -> THE fix for CAJOE signal detection: internal pull-up/-down chokes
        the high-impedance P3-VIN tap (10k in series). 'ip,pn' = input,
        pull none — from boot, independent of the script.
  4. Download the BIP39 wordlist english.txt and verify it via SHA-256
  5. Check the environment: /dev/hwrng, I2C bus/camera (0x33), rfkill
  6. If btc_seedgen.py sits next to it: syntax check + self-test

Run:    sudo python3 install_seedgen.py
Then:   reboot once (I2C + GPIO configuration become active at boot),
        then  sudo python3 btc_seedgen.py"""
import hashlib
import os
import shutil
import subprocess
...
def print_pinout():
    """Complete 40-pin map with orientation + wiring of both sensors."""
    ...
    Z = [
     ...
     (C+"CAJOE Signal: P3-VIN-Abgriff"+R_, C, "GPIO17  ","11", "12", "GPIO18", D, ""),
    ...
    print("   • Signalabgriff an " + C + "P3-VIN" + R_ + ": weisses Kabel mit " +
          C + "10 kOhm IN REIHE" + R_ + " zu GPIO17 (Pin 11).")
    print("   • DER entscheidende Punkt ist die Software-Einstellung: GPIO17 muss")
    print("     " + C + "biasfrei" + R_ + " laufen (Pull None) — jeder interne Pull-up/-down")
    print("     wuergt den hochohmigen Abgriff ab. Dieses Skript verankert das")
    print("     dauerhaft (gpio=17=ip,pn in config.txt).")
...
def main():
    ...
    # ---- 3) GPIO17 bias-free (THE CAJOE signal fix) --------------------------
    kopf(3, f"GPIO{GPIO_GEIGER} dauerhaft biasfrei konfigurieren")
    print(DIM + "     Hintergrund: Der P3-VIN-Abgriff (mit 10k in Reihe) ist hochohmig —"
          "\n     jeder interne Pull-up/-down des Pi wuergt das Signal ab. 'ip,pn'"
          "\n     setzt den Pin ab Boot auf Input ohne Pull (DER Fix der Signalerkennung)." + RST)
    cfg = config_txt_pfad()
    if cfg:
        inhalt = open(cfg).read()
        ...
        # additionally set it right away if pinctrl exists (usable without reboot)
    ...
    # ---- 6) Test the seed generator (if present) -----------------------------

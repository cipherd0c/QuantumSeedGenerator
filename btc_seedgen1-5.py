    ...
    for n, h, label in kandidaten:
        if handle is None:
            try:
                ...
                # audit-46 HW-3: SET_PULL_UP statt SET_PULL_NONE — der CAJOE-
                # INT-Ausgang ist je nach Platinenrevision Open-Collector
                # (idle HIGH, LOW-aktiver Puls); ohne Pull wuerde die Leitung
                # schwimmen. Bei aktiv getriebenem Ausgang ist der interne
                # Pull-up (~50 kOhm) unschaedlich, die Auto-Flankenerkennung
                # bleibt korrekt.
                lgpio.gpio_claim_input(h, gpio_nr, lgpio.SET_PULL_UP)
                ruhe = lgpio.gpio_read(h, gpio_nr)
                lgpio.gpio_free(h, gpio_nr)
                edge = lgpio.RISING_EDGE if ruhe == 0 else lgpio.FALLING_EDGE
                lgpio.gpio_claim_alert(h, gpio_nr, edge, lgpio.SET_PULL_UP)

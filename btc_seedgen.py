...
only; the script never writes a file. See the project documentation for
...
def find_rp1_chip(lgpio, gpio_nr):
    ...
    for n in range(0, 32):
        ...
        if "rp1" in label or "54" in label.replace(",", " ").split():
            kandidaten.insert(0, (n, h))      # prefer rp1 chips at the front
        else:
...
def collect_decay(n_events: int, mock: bool, transparent: bool = False) -> SourceResult:
    ...
    if mock:
        # simulated Poisson process ~30 CPM (FOR TESTING ONLY)
        import random
...
def combine(hw: SourceResult, dec, cam,
    ...
    mode_tag = b"light" if light else b"full"               # domain separation of the modes
...
def selftest():
    ...
    # (combine mixes in os.urandom -> two calls must differ)
    ...
    # 8) verify the wordlist (if present)
...
def check_remote_session(args):
    ...
    client's scrollback; tmux/screen can record sessions."""
    if args.mock:
        return
...
def startup_healthcheck(mock: bool, light: bool = False, interactive: bool = True):
    ...
    if not mock and cpm_mess < CPM_WARN_MIN:
        if _low_cpm_dialog(cpm_mess, interactive):
            ...
            return startup_healthcheck(mock, light, interactive)
...
def main():
    ...
    if words_list and not args.mock:
        ...
        if args.mock:
            # audit N3: mock seeds must not be transcribable -> indices only

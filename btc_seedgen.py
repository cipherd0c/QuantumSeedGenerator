...
# Konstanten
# ----------------------------------------------------------------------------
VERSION            = "1.5.1"   # v1.5: Audit-46-Befunde; v1.5.1: GPIO-Pull-Ruecknahme + RAM-Haertung
...
GPIO_PULL_MODE     = "none"  # HARDWARE-VERIFIZIERT, NICHT AENDERN ohne Test am
                            # realen Geraet: Der CAJOE-Signalabgriff an P3-VIN
                            # laeuft ueber 10 kOhm in Reihe und ist damit
                            # HOCHOHMIG — jeder interne Pull-up/-down (~50 kOhm)
                            # wuergt das Signal ab, es kommt KEIN einziger Puls
                            # an. Das wurde in der Projekthistorie muehsam am
                            # Geraet gefunden; install_seedgen.py verankert es
                            # zusaetzlich dauerhaft mit 'gpio=17=ip,pn' in der
                            # config.txt. ACHTUNG: gpio_claim_input() ueber-
                            # schreibt diese Boot-Einstellung zur Laufzeit —
                            # das Skript MUSS hier ebenfalls "none" setzen.
                            #
                            # Audit-46 HW-3 hatte "up" empfohlen (Open-Collector-
                            # Argument aus dem Datenblatt) und v1.5 stellte um;
                            # audit-47 hat das als Feldfehler zurueckgenommen.
                            # Per --gpio-pull auf up/down umstellbar, falls eine
                            # Platinenrevision einen echt schwimmenden Ausgang
                            # hat; der Healthcheck probiert die Modi ohnehin
                            # automatisch durch.
...
def _pull_konstante(lgpio, modus: str):
    ...
    return {"up": lgpio.SET_PULL_UP,
            ...
            "down": lgpio.SET_PULL_DOWN}.get(modus, lgpio.SET_PULL_NONE)
    ...
    for n, h, label in kandidaten:
        if handle is None:
            try:
                ...
                # Pull siehe GPIO_PULL_MODE: Standard "none" (biasfrei), weil
                # der hochohmige P3-VIN-Abgriff mit 10k in Reihe von jedem
                # internen Pull abgewuergt wird — am Geraet verifiziert.
                # gpio_claim_input ueberschreibt die config.txt-Einstellung
                # 'gpio=17=ip,pn', deshalb muss der Wert hier passen.
...
def _startup_healthcheck_once(mock: bool, light: bool = False, interactive: bool = True):
    ...
    if mock:
        ...
        _pull_kandidaten = [GPIO_PULL_MODE] + [m for m in ("none", "up", "down")
                                               if m != GPIO_PULL_MODE]
...
def main():
    ...
    ap.add_argument("--gpio-pull", choices=("up", "none", "down"), default="none",
                    help="Pull-Widerstand am CAJOE-Eingang GPIO17. Standard "
                         "'none' (biasfrei) — hardwareverifizierter Fix: der "
                         "hochohmige P3-VIN-Abgriff (10k in Reihe) wird von "
                         "jedem internen Pull abgewuergt, passend zu "
                         "'gpio=17=ip,pn' aus install_seedgen.py. 'up'/'down' "
                         "nur bei abweichender Platinenrevision; der "
                         "Healthcheck probiert die Modi ohnehin automatisch "
                         "durch.")
    args = ap.parse_args()

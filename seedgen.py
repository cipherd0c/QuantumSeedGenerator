...
DECAY_TIMEOUT_EFF  = 360      # audit F2-M4: rate-adaptive (set after the health
                              # check: max(360, ~10 mean intervals) — at 1 CPM a
...
DICE_MIN_ROLLS_LIGHT  = 104   # audit F3-H3: 104 x log2(6) + 256 = 524.8 >= 2x256
...
def _decay_stats_check(timestamps, mock: bool):
    ...
    if not mock:
        cpm = 60.0 / m
        if not (0.3 <= cpm <= 3000.0):   # audit F3-N3: 0.5 edge rejected honest 0.5 CPM tubes ~49%
            ...
            fail_abort("Radiozerfall (CAJOE)",
                       t(f"Zaehlrate {cpm:.0f} CPM ausserhalb des Plausibilitaetsfensters "
                         "0.3-3000 — Stoerquelle statt Roehre? (Audit M4)",
                         f"count rate {cpm:.0f} CPM outside plausibility window "
                         "0.3-3000 — interference instead of tube? (audit M4)"))
...
def collect_camera(n_frames: int, mock: bool, transparent: bool = False) -> SourceResult:
    ...
    if mock:
        ...
        try:
            ...
            # audit F3-M5: an EEPROM-flashed 64 Hz sensor tears virtually
            # every frame silently (read ~200 ms >> subpage 15.6 ms). Verify
            # the control register once; set 2 Hz if it deviates.
            try:
                ...
                if rate_bits != 0b010:      # power-on default: 2 Hz
                    ...
                    print(C.YEL + t(f"   ⚠ Refresh-Rate stand auf {rate_bits:03b} — auf 2 Hz gesetzt.",
                                    f"   ⚠ refresh rate was {rate_bits:03b} — set to 2 Hz.") + C.RESET)
            except Exception:
...
def check_remote_session(args):
    ...
    if os.environ.get("TMUX") or os.environ.get("STY") or ("tmux" in ahnen) or ("screen" in ahnen):
        ...
        print(C.RED + C.BOLD + t(
            " ⚠ tmux/screen erkannt: Die Scrollback-Loeschung erreicht deren "
            "Puffer/Logs NICHT zuverlaessig — der Seed koennte dort verbleiben!",
            " ⚠ tmux/screen detected: the scrollback erase does NOT reliably reach "
...
def enforce_permanent_radio_off(args):
    ...
    for z in (z.strip() for z in inhalt.split("\n")):
        ...
        if sektion in ("all", "pi5"):
            if z.startswith("dtoverlay=disable-wifi"):
                hat_wifi = True
            elif z.startswith("dtoverlay=disable-bt"):
                hat_bt = True
    ...
    try:
        ...
        for n in os.listdir("/sys/class/net"):
            ...
            try:
                ...
                continue
            if oper == "down":
                subprocess.run(["ip", "link", "set", n, "up"], capture_output=True)
                geweckt.append(n)
        if geweckt:
...
def final_offline_check(args):
    ...
    if not keep:
        for typ in sorted(_sys_rfkill_unblocked_types() - {"wlan", "bluetooth"}):
        ...
        try:
            ...
            while gezeigt < 10:
                ...
                while gezeigt < min(len(ts_list), 10):
                    ...
                    gezeigt += 1
                    letzte_aktivitaet = time.monotonic()
                if time.monotonic() - letzte_aktivitaet > DECAY_TIMEOUT_S * 4:   # audit F3-M3: 0.5 CPM legal -> 1440 s
                    fail_abort("Radiozerfall (Initialisierungstest)",
                               f"{DECAY_TIMEOUT_S * 4}s ohne Zerfallsereignis.")
        except KeyboardInterrupt:
            print(C.RED + t("\n Abbruch durch Benutzer (Strg+C).",
                            "\n aborted by user (Ctrl+C).") + C.RESET)
...
def main():
    ...
    if not args.yes:
        ...
        if args.light:
            print(C.DIM + t("   LIGHT: Die Sicherheitslast traegt der HMAC-Kombinierer — Minimum 104 Wuerfe.",
                            "   LIGHT: the HMAC combiner carries the security — minimum 104 rolls.") + C.RESET)
